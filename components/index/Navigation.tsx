...
const Navigation = ({time}:any) => {
    ...
    // 로그아웃
    const doLogout = () => {
        window.open('https://api-v2.storicha.in/User/Logout')
    ...
    React.useEffect(()=>{
        ...
        if(router.asPath.includes('/wlogin?q=')){
            axios({
                ...
                data: {
                    tk: router.asPath.replace('/wlogin?q=', ''),
                },
            })
            .then((response) => {
                localStorage.setItem('stori-token', router.asPath.replace('/wlogin?q=', ''))
                localStorage.setItem('stori-email',response.data.response_data[0].email)
                localStorage.setItem('stori-nickName',response.data.response_data[0].nick_name)
                localStorage.setItem('stori-countryCode',response.data.response_data[0].country_code)
                setLogin(response.data.response_data[0].email)
                router.push('/')
            })
            .catch((error)=> {
                console.log(error)
                ...
                <div className="right">
                    {login !== '' && login !== null ? (
                        <div className="right-container" onClick={seeLoginMenuState}>
                            ...
                            <p className='text-xl pl-2'>{login}</p>
                        ...
                        <p className="login" onClick={()=> window.open(`https://api-v2.storicha.in/user/login?ReturnUrl=${encodeURIComponent(window.location.host)}devipstudio`)}>로그인</p>
                    )}
                </div>
            ...
            <Container ref={elem => (wrapperRef.current[1] = elem)} color={isPointTheme} style={leftView ? {left: '0px'} : {left: '-100%'}}> <MenuBox color={isPointTheme}>
                    ...
                    <PlusMenu color={isPointTheme}>
                        ...
                        <div className='content-box' style={claim ? {maxHeight: '100vh', display: 'flex', padding: '8px'} : {}}>
                            ...
                            <div className='content' onClick={()=> router.push('sale/episode?idx=5737')}>
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" />
                                </svg>
                                <p>라이센스</p>
                        ...
                        <div className={`content-box ${more && 'open'}`} style={more ? {display:'flex', maxHeight: '100vh'}: {}}>
                            ...
                            <div className='content' onClick={()=> Router.push('/series?idx=5737')}>
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                ...
                >
                    <div>
                        ...
                        <h4><b>{login === '' ? 'Please, Login' : login}</b><br/>{login === '' ? 'Please, Login' : login}</h4>
                    </div>
                </ProfileBox>
            </Container>
        </>
